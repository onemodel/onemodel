    ...
    */
    ...
    //) -> Option<DateTime<FixedOffset>> {
        ...
        loop {
            ...
            let ans = ui.ask_for_string3(leading_text, None, default.to_string());
            match ans {
                None => return None,
                Some(answer) => {
    ...
    pub fn ask_for_relation_directionality(
        ...
        let directionality = ui.ask_for_string3(
            msg,
            Some(criteria_fn),
            previous_directionality_in.to_string(),
        );
    ...
    pub fn prompt_whether_to_1add_or_2correct(attr_type_desc_in: &str, ui: &TextUI) -> Result<Option<i32>, anyhow::Error> {
        loop {
            let answer = ui.ask_which(
                None,
                &vec![
                    format!("1-Save this {} attribute?", attr_type_desc_in),
                    "2-Correct it?".to_string(),
                ],
                &vec![],
                true,
                None,
