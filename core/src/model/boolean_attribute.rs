...
impl BooleanAttribute<'_> {
    ...
    fn read_data_from_db(&mut self) -> Result<(), String>  {
        ...
        /*DataType::Bigint(%%)*/ self.m_valid_on_date = None; //ba_type_data[3];
...
*/
  fn update(&mut self, attr_type_id_in: i64, boolean_in: bool, valid_on_date_in: Option<i64>, observation_date_in: i64) -> Result<(), String> {
    // write it to the database table--w/ a record for all these attributes plus a key indicating which Entity
    // it all goes with
    self.m_db.update_boolean_attribute(self.m_id, self.get_parent_id()?, attr_type_id_in, boolean_in, valid_on_date_in, observation_date_in);
