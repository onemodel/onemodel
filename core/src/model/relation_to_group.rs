...
///See comments on fn new, here.
    ...
    fn update(&mut self, transaction: &Option<&mut Transaction<Postgres>>,
              new_relation_type_id_in: Option<i64>, new_group_id_in: Option<i64>,
              valid_on_date_in:Option<i64>, observation_date_in:Option<i64>) -> Result<u64, anyhow::Error> {
  ...
  fn read_data_from_db(
      ...
      if data.len() == 0 {
          return Err(anyhow!(
                ...
                self.entity_id, self.rel_type_id, self.group_id
            ));
      }
