...
impl PostgreSQLDatabase {
    ...
    fn get_sorted_attributes(
    ...
    ) -> Result<(Vec<(i64, Box<dyn Attribute>)>, usize), anyhow::Error> {
        let mut all_results: Vec<(Option<i64>, Box<dyn Attribute>)> = Vec::new();
        ...
        while table_list_index < tables.len() {
            ...
            for result in results {
                ...
                if table_name == Util::QUANTITY_TYPE {
                    //let sorting_index = match result[0] {
                    //    None => None,
                    //    Some(DataType::Bigint(si)) => Some(si),
                    //    _ => return Err(anyhow!("Unexpected value: result[0]: {:?}", result[0])),
                    //};
                    let sorting_index = Util::get_value_bigint_option("result[0]", result[0])?,
                    //if result[0].is_none() { None }
                    //else { Some(result[0].unwrap().as_bigint()?) };
                    all_results.push((
                        sorting_index,
                        Box::new(QuantityAttribute::new(
                            db.clone(),
                            //result[1].clone().unwrap().as_bigint()?,
                            //match result[1] {
                            //    Some(DataType::Bigint(x)) => x,
                            //    _ => return Err(anyhow!( "Unexpected value for result[1]: {:?}", result[1])),
                            //},
                            Util::get_value_bigint("result[1]", result[1])?,
                            //result[2].clone().unwrap().as_bigint()?,
                            //match result[2] {
                            //    Some(DataType::Bigint(x)) => x,
                            //    _ => return Err(anyhow!( "Unexpected value for result[2]: {:?}", result[2])),
                            //},
                            Util::get_value_bigint("result[2]", result[2])?,
                            //result[3].clone().unwrap().as_bigint()?,
                            //match result[3] {
                            //    Some(DataType::Bigint(x)) => x,
                            //    _ => return Err(anyhow!( "Unexpected value for result[3]: {:?}", result[1])),
                            //},
                            Util::get_value_bigint("result[3]", result[1])?,
                            //result[4].clone().unwrap().as_bigint()?,
                            //match result[4] {
                            //    Some(DataType::Bigint(x)) => x,
                            //    _ => return Err(anyhow!( "Unexpected value for result[4]: {:?}", result[4])),
                            //},
                            Util::get_value_bigint("result[4]", result[4])?,
                            //result[5].clone().unwrap().as_float()?,
                            //match result[5] {
                            //    Some(DataType::Float(x)) => x,
                            //    _ => return Err(anyhow!( "Unexpected value for result[5]: {:?}", result[5])),
                            //},
                            ...
                            //    None => None,
                            //    _ => return Err(anyhow!( "Unexpected value for result[6]: {:?}", result[6])),
                            //},
                            Util::get_value_bigint_option("result[6]", result[6])?,
                            //result[7].clone().unwrap().as_bigint()?,
                            //match result[7] {
                            //    Some(DataType::Bigint(x)) => x,
                            //    _ => return Err(anyhow!( "Unexpected value for result[7]: {:?}", result[7])),
                            //},
                            Util::get_value_bigint("result[7]", result[7])?,
                            //result[0].clone().unwrap().as_bigint()?,
                            //match result[0] {
                            //    Some(DataType::Bigint(x)) => x,
                            //    _ => return Err(anyhow!( "Unexpected value for result[0]: {:?}", result[0])),
                            //},
                            Util::get_value_bigint_option("result[0]", result[0])?,
                        )),
                    ));
                } else if table_name == Util::TEXT_TYPE {
                    //let sorting_index = if result[0].is_none() {
                    //    None
                    //} else {
                    //    Some(result[0].clone().unwrap().as_bigint()?)
                    //};
                    let sorting_index = Util::get_value_bigint_option("result[0]", result[0])?,
                    all_results.push((
                        sorting_index,
                        Box::new(TextAttribute::new(
                        //Attribute::TextAttribute(TextAttribute::new(
                            db.clone(),
                            //result[1].clone().unwrap().as_bigint()?,
                            //result[2].clone().unwrap().as_bigint()?,
                            //result[3].clone().unwrap().as_bigint()?,
                            ...
                            //    None
                            //} else {
                            //    Some(result[5].clone().unwrap().as_bigint()?)
                            //},
                            //result[6].clone().unwrap().as_bigint()?,
                            //result[0].clone().unwrap().as_bigint()?,
                            Util::get_value_bigint("result[1]", result[1])?,
                            Util::get_value_bigint("result[2]", result[2])?,
                    ...
                    all_results.push((
                        sorting_index,
                        Attribute::DateAttribute(DateAttribute::new(
                            self.clone(),
                            result[1].clone().unwrap().as_bigint()?,
                            result[2].clone().unwrap().as_bigint()?,
                            result[3].clone().unwrap().as_bigint()?,
                    ...
                    all_results.push((
                        sorting_index,
                        Attribute::BooleanAttribute(BooleanAttribute::new(
                            self.clone(),
                            result[1].clone().unwrap().as_bigint()?,
                            result[2].clone().unwrap().as_bigint()?,
                            result[3].clone().unwrap().as_bigint()?,
                            result[4].clone().unwrap().as_bool()?,
                            if result[5].is_none() {
                                None
                            } else {
                                Some(result[5].clone().unwrap().as_bigint()?)
                    ...
                    all_results.push((
                        sorting_index,
                        Attribute::FileAttribute(FileAttribute::new(
                            self.clone(),
                            result[1].clone().unwrap().as_bigint()?,
                            result[2].clone().unwrap().as_bigint()?,
                            result[3].clone().unwrap().as_bigint()?,
                            ...
                            result[6].clone().unwrap().as_bigint()?,
                            result[7].clone().unwrap().as_string()?,
                            result[8].clone().unwrap().as_bool()?,
                            result[9].clone().unwrap().as_bool()?,
                            result[10].clone().unwrap().as_bool()?,
                            result[11].clone().unwrap().as_bigint()?,
                            result[12].clone().unwrap().as_string()?,
                            result[0].clone().unwrap().as_bigint()?,
                        )?),
                    ));
                    ...
                    all_results.push((
                        sorting_index,
                        Attribute::RelationToLocalEntity(RelationToLocalEntity::new(
                            self.clone(),
                            result[1].clone().unwrap().as_bigint()?,
                            result[2].clone().unwrap().as_bigint()?,
                            result[3].clone().unwrap().as_bigint()?,
                    ...
                    all_results.push((
                        sorting_index,
                        Attribute::RelationToGroup(RelationToGroup::new(
                            self.clone(),
                            result[1].clone().unwrap().as_bigint()?,
                            result[2].clone().unwrap().as_bigint()?,
                            result[3].clone().unwrap().as_bigint()?,
                    ...
                    all_results.push((
                        sorting_index,
                        Attribute::RelationToRemoteEntity(RelationToRemoteEntity::new(
                            self.clone(),
                            result[1].clone().unwrap().as_bigint()?,
                            result[2].clone().unwrap().as_bigint()?,
                            result[3].clone().unwrap().as_bigint()?,
        ...
        let mut all_results_array: Vec<(i64, Box<dyn Attribute>)> =
        ...
        let from = starting_object_index_in;
        ...
        Ok((
            all_results_array[from..until].to_vec(),
