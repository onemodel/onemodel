...
impl Controller {
    ...
    fn copy_and_edit_attributes(
        &self,
        entity_in: &Entity,
        mut template_attributes_to_copy_in: Vec<Rc<RefCell<dyn Attribute>>>,
    ) -> Result<(), anyhow::Error> {
        ...
        for mut attribute_from_template in template_attributes_to_copy_in.clone() {
            ...
            let new_attribute: Option<Box<dyn Attribute>> = match attribute_from_template
                .borrow().get_form_id()?
            {
                form_id if self.db.borrow().get_attribute_form_name(form_id)? == Util::QUANTITY_TYPE => {
                    prompt_to_edit_attribute_copy();
                    let mut template_attr = (&attribute_from_template as &dyn Any)
                ...
                form_id if self.db.borrow().get_attribute_form_name(form_id)? == Util::DATE_TYPE => {
                    ...
                    let template_attr = (&attribute_from_template as &dyn Any)
                        .downcast_ref::<DateAttribute>()
                        .unwrap();
                ...
                form_id if self.db.borrow().get_attribute_form_name(form_id)? == Util::RELATION_TO_GROUP_TYPE => {
                    prompt_to_edit_attribute_copy();
                    //%%?:
                    let mut template_attr_opt: Option<&mut RelationToGroup> = (&attribute_from_template as &dyn Any)
                        .downcast_mut::<RelationToGroup>();
                    let Some(mut template_attr) = template_attr_opt else {
    ...
    fn copy_and_edit_relation_to_entity(
        ...
        if ask_every_time.is_none() {
            ...
            let how_handle_rtes_choices = vec![
                format!("For ALL entity relations being added: {}", choice1_text).as_str(),
                format!("For ALL entity relations being added: {}", choice2_text).as_str(),
                format!("For ALL entity relations being added: {}", choice3_text).as_str(),
            ...
            for (_, cde_attribute) in cde_attribute_tuples {
                ...
                for (_, attribute) in existing_attribute_tuples_in {
                    if !attribute_type_found_on_entity {
                        let type_id = attribute.borrow().get_attr_type_id(None)?;
