...
//%%where
    ...
    pub fn find_existing_object_by_text(
        ...
        let leading_text: Vec<&str> = vec![x.as_str()];
    ...
    pub fn choose_or_create_object(
        ...
        let leading_text_as_strs: Vec<&str> = leading_text.iter().map(|s| s.as_str()).collect();
            ...
            && answer <= choices.len()
        {
            let result: Option<i64> =
                ...
                self.ask_for_and_write_class_and_template_entity_name(db_in, None, &None, &None, Util::ENTITY_CLASS_TYPE, 
